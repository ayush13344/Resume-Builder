...
import PersonelInfoForm from "../components/PersonelInfoForm";
import ResumePreview from "../components/ResumePreview";
import TemplateSelector from "../components/TemplateSelector";
import ColorPicker from "../components/ColorPicker";
import ProfSummaryForm from "../components/ProfSummaryForm";
...
const ResumBuilder = () => {
    ...
    useEffect(() => {
  console.log("Project:", resumeData.project);
}, [resumeData.project]);
    ...
    const changeResumeVisibility=async()=>{
        try{
            ...
            console.log("Error updating resume visibility:",error);
        }
    }
    ...
    return (
        <div>
            <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                hover: text-slate-700 transition-all"> <ArrowLeftIcon className="size-4" />Back to dashboard </Link>
            </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1">
                            ...
                            <div className="flex justify-between items-center mb-6 border-b border-gray-300 py-1">
                                ...
                                <div className="flex items-center">
                                    ...
                                    <button onClick={()=>setActiveSectionIndex((prevIndex)=>Math.min(prevIndex+1,sections.length-1))} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50
                                        transition-all ${activeSectionIndex === sections.length - 1 && "opacity-50"}`} disabled={activeSectionIndex === sections.length - 1}
                                        >
                            ...
                            <div className="space-y-6">
                                        {activeSection.id==='personal' && (
                                           <PersonelInfoForm data={resumeData.personal_info} onChange={(data)=>setResumeData(prev => ({...prev,personal_info:data}))} removeBackground={removeBackground} setRemoveBackground={setRemoveBackground} />
                                        )}
                                        {
                                          activeSection.id==='summary' && (
                                            <ProfSummaryForm data={resumeData.professional_summary} onChange={(data)=>setResumeData(prev=>({...prev,professional_summary:data}))} setResumeData={setResumeData}  />
                                          )  
                                        }
                                        {
                                        ...
                                        <div className="relative w-full">
                                            <div className="absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2">
                                                {resumeData.public && (
                                                    ...
                                                    from-blue-100 to-blue-200 text-blue-600 rounded-lg ring-lg ring-blue-300 hover:ring transition-colors"
                                                    >
                                                ...
                                                <button onClick={downloadResume} className="flex items-center gap-2 px-6 py-2  text-xs bg-gradient-to-br fom-green-100 to-green-200 text-green-600 rounded-lg ring-green-300 hover:ring transition-colors">
