                    ...
                    {data.map((education,index)=>(
                        <div className="p-4 border border-gray-200 rounded-lg space-y-3" key={index}>
                            ...
                            <div className="grid md:grid-cols-2 gap-3">
                                ...
                                className="px-3 py-2 text-sm" onChange={(e)=>updateEducation(index,"institution",e.target.value)} />

                                 <input type="text" value={education.degree || ""} placeholder="Degree (eg. B.Sc in Computer Science)"
                                className="px-3 py-2 text-sm " onChange={(e)=>updateEducation(index,"degree",e.target.value)} />

                                 <input type="text" value={education.field || ""} placeholder="Field of Study"
                                className="px-3 py-2 text-sm " onChange={(e)=>updateEducation(index,"field",e.target.value)} />

                                 <input type="month" value={education.graduation_date || ""} 
                                className="px-3 py-2 text-sm  " onChange={(e)=>updateEducation(index,"graduation_date",e.target.value)} />
                            </div>
                            <input type="text" value={education.gpa || ""} placeholder="GPA (optional)"
                                className="px-3 py-2 text-sm " onChange={(e)=>updateEducation(index,"gpa",e.target.value)} />
